...
if(conf.use_viewer):
    if(conf.system=='ur-lab' and conf.which_viewer=="meshcat"):
        lab.display_disi_lab_meshcat(simu)
    elif(conf.system=='ur-lab'):
    ...
    if(conf.weight_final_ee_pos>0):
        if conf.which_viewer == "meshcat":
            ...
            target_sphere = simu.viz.viewer["world/target"].set_object(g.Sphere(0.05), g.MeshBasicMaterial(color=0xda8a00, opacity=1))
            target_sphere = simu.viz.viewer["world/target"]
            target_sphere.set_transform(tf.translation_matrix(conf.p_des.tolist()))
        else:
