...
def backward_pass(solver, X_bar, U_bar, mu):
        ...
        for i in range(N-1, -1, -1):
            ...
            Qbar_uu       = solver.Q_uu[i, :, :]  + mu * np.eye(solver.Q_uu[i].shape[0])
            ...
            V_x[i,:]    = (solver.Q_x[i,:].T + 
                solver.Q_u[i,:].T @ solver.K[i,:,:] +
                solver.w[i,:].T @ solver.Q_uu[i, :, :] @ solver.K[i, :, :] + 
                solver.w[i,:].T @ solver.Q_xu[i,:,:].T)
