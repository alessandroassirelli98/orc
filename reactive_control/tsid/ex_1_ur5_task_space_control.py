...
if viz is not None:
    target_sphere = viz.viewer['debug/target_sphere'].set_object(g.Sphere(conf.SPHERE_RADIUS), g.MeshBasicMaterial(color=0x0BFFAA, opacity=0.5))
    position_sphere = viz.viewer['debug/position_sphere'].set_object(g.Sphere(conf.SPHERE_RADIUS), g.MeshBasicMaterial(color=0xF3D964))
    
    target_sphere = viz.viewer['debug/target_sphere']
    position_sphere = viz.viewer['debug/position_sphere']
...
for i in range(0, N):
    ...
    if i%conf.DISPLAY_N == 0: 
        viz.display(q[:,i])
        position_sphere.set_transform(tf.translation_matrix(ee_pos[:,i]))
        target_sphere.set_transform(tf.translation_matrix(ee_pos_ref[:,i]))
