...
class DDPSolverLinearDyn(DDPSolver):
    ...
    def cost_running(self, i, x, u):                                    
        ...
        cost = 0.5*np.dot(x, np.dot(self.H_xx[i,:,:], x)) \
                ...
                + 0.5 * underact * u[1]**2 #add here the running cost term for taking into account the underactuation
    ...
    def cost_running_u(self, i, x, u):
        ...
        c_u = self.lmbda * u \
            + np.array([0, underact * u[1]]) # + ... add here the derivative w.r.t u of the running cost term for taking into account the underactuation                            
        if self.CONTROL_BOUNDS:
            pass
