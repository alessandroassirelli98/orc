...
class DDPSolverLinearDyn(DDPSolver):
    ...
    def cost_running(self, i, x, u):
        ...
        cost = (
            0.5 * np.dot(x, np.dot(self.H_xx[i, :, :], x))
            + np.dot(self.h_x[i, :].T, x)
            + self.h_s[i]
    ...
    def cost_final(self, x):
        ...
        cost = (
            0.5 * np.dot(x, np.dot(self.H_xx[-1, :, :], x))
            + np.dot(self.h_x[-1, :].T, x)
            + self.h_s[-1]
        )
        return cost
    ...
    def cost_running_uu(self, i, x, u):
        ...
        c_uu = self.lmbda * np.eye(self.nu) + np.matrix(
            [[0, 0], [0, underact]]
...
class DDPSolverDoublePendulum(DDPSolverLinearDyn):
    ...
    def f_x(self, x, u):
        ...
        if conf.SELECTION_MATRIX == 1 and conf.ACTUATION_PENALTY == 0:
            S = np.array(
                [[1.0, 0.0], [0.0, 0.0]]
            )  # Selection matrix for taking into account underactuation
            self.Fu[nv:, :] = data.Minv.dot(
                S.T
            ...
            raise RuntimeError(
                "No method has been chosen to consider the underactuated case"
            )
...
if __name__ == "__main__":
    ...
    if conf.TORQUE_LIMITS:
        solver = DDPSolverDoublePendulum(
            "doublependulum",
            robot,
            ddp_params,
            H_xx,
    ...
    X_sim, U_sim = solver.start_simu(
        x0, X, U, KK, conf.dt_sim, conf.PUSH, conf.TORQUE_LIMITS
    )
    ...
    if conf.PLOT_TORQUES:
        ...
        if conf.TORQUE_LIMITS:
            plt.plot(
                time_vec[:-1],
                max_torque * np.ones(len(time_vec[:-1])),
                "k--",
                alpha=0.8,
                linewidth=1.5,
            )
        ...
        plt.axvline(int(N / 8) * conf.dt, linestyle="dashed", color="black")
        plt.axvline(int(N / 4) * conf.dt, linestyle="dashed", color="black")
        plt.axvline(int(N / 2) * conf.dt, linestyle="dashed", color="black")
        plt.gca().set_xlabel("Time [s]")
        plt.gca().set_ylabel("[Nm]")
        leg = plt.legend(
            ["1st joint torque", "1st joint ref. torque"], loc="upper right"
        )
    ...
    if conf.PLOT_JOINT_POS:
        ...
        plt.legend(
            [
                "1st joint position",
                "1st joint ref. position",
                "2nd joint position",
                "2nd joint ref position",
            ],
            loc="upper right",
        )
    ...
    if conf.PLOT_JOINT_VEL:
        ...
        plt.legend(
            [
                "1st joint velocity",
                "1st joint ref. velocity",
                "2nd joint velocity",
                "2nd joint ref velocity",
            ],
            loc="upper right",
        )
