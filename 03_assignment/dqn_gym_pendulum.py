...
MODEL_NAME = "gym_pendulum_3ndu_uMax2"
...
for episode in range (1, MAX_NUMBER_OF_EPISODES):
    ...
    while not done:
        ...
        next_state, reward, done, _ = env.step(control_map[iu])
...
while not done:
    action = np.array(control_map[np.argmin(agent.model.predict(current_state))])
